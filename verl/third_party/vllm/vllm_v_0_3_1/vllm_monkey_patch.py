...
class LLM:
    ...
    def generate(
        ...
        print(f"[Timing][VLLM] Parsed prompts in {time.time() - t0:.4f} seconds")
        ...
        print(f"[Timing][VLLM] Processed guided decoding request in {time.time() - t0:.4f} seconds")

        t0 = time.time()
        if sampling_params is None:
            sampling_params = self.get_default_sampling_params()
        print(f"[Timing][VLLM] Processed sampling parameters in {time.time() - t0:.4f} seconds")
        ...
        self._validate_and_add_requests(
            ...
            priority=priority)
        print(f"[Timing][VLLM] validate_and_add_requests took {time.time() - t0:.4f} seconds")

        t0 = time.time()
        outputs = self._run_engine(use_tqdm=use_tqdm)
        print(f"[Timing][VLLM] _run_engine took {time.time() - t0:.4f} seconds")

        print(f"[Timing][VLLM] generate() total time: {time.time() - start_total:.4f} seconds")
