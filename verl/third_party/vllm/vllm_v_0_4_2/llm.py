...
class LLM:
    ...
    def generate(
        ...
        print(f"[Timing][VLLM-Generate-MM] Checked inputs and normalized prompts in {time.time() - t0:.4f} seconds")
        ...
        print(f"[Timing][VLLM-Generate-MM] Determined num_requests={num_requests} in {time.time() - t0:.4f} seconds")
        ...
        print(f"[Timing][VLLM-Generate-MM] Processed sampling parameters in {time.time() - t0:.4f} seconds")

        t0 = time.time()
        if multi_modal_data:
            multi_modal_data.data = multi_modal_data.data.to(torch.float16)
            print(f"[Timing][VLLM-Generate-MM] Casted multi-modal data to float16 in {time.time() - t0:.4f} seconds")
        ...
        print(f"[Timing][VLLM-Generate-MM] Added all {num_requests} requests in {time.time() - t_add:.4f} seconds")

        t0 = time.time()
        result = self._run_engine(use_tqdm)
        print(f"[Timing][VLLM-Generate-MM] Ran engine and generated outputs in {time.time() - t0:.4f} seconds")

        print(f"[Timing][VLLM-Generate-MM] Total time for generate: {time.time() - start_total:.4f} seconds")
        return result
