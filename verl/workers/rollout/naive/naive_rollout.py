...
class NaiveRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        print(f"[Timing][HF] Extracted tensors and meta info in {time.time() - t0:.4f} seconds")

        t0 = time.time()
        self.module.eval()
        print(f"[Timing][HF] Set module to eval mode in {time.time() - t0:.4f} seconds")

        t0 = time.time()
        prev_attention_mask = torch.ones(size=(batch_size, 1), dtype=attention_mask.dtype, device=attention_mask.device)
        print(f"[Timing][HF] Initialized prev_attention_mask in {time.time() - t0:.4f} seconds")
        ...
        for step in range(self.config.response_length):
            ...
            print(
                f"[Timing][HF] Step {step + 1}/{self.config.response_length} completed in {time.time() - t_step:.4f} seconds")

        print(f"[Timing][HF] Completed response generation loop in {time.time() - t_loop:.4f} seconds")

        t0 = time.time()
        logits = torch.stack(logits_lst, dim=1)
        print(f"[Timing][HF] Stacked logits in {time.time() - t0:.4f} seconds")

        t0 = time.time()
        prompts = idx[:, :prompt_length]
        response = idx[:, prompt_length:]
        print(f"[Timing][HF] Split sequences into prompts and responses in {time.time() - t0:.4f} seconds")

        t0 = time.time()
        log_probs = logprobs_from_logits(logits=logits, labels=response)
        print(f"[Timing][HF] Computed log_probs in {time.time() - t0:.4f} seconds")
        ...
        print(f"[Timing][HF] Created output TensorDict in {time.time() - t0:.4f} seconds")

        t0 = time.time()
        self.module.train()
        print(f"[Timing][HF] Set module back to train mode in {time.time() - t0:.4f} seconds")

        print(f"[Timing][HF] Total time for generate_sequences: {time.time() - start_total:.4f} seconds")
